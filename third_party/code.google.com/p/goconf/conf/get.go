...
// GetInt64 has the same behaviour as GetString but converts the response to int.
...
// GetUint64 has the same behaviour as GetString but converts the response to int.
...
// GetSecond has the same behaviour as GetString but converts the response to time.Second.
func (c *ConfigFile) GetSecond(section string, option string) (value time.Duration, err error) {
	sv, err := c.GetString(section, option)
	if err == nil {
		second, err := strconv.Atoi(sv)
		if err != nil {
			err = GetError{CouldNotParse, "int", sv, section, option}
		}
		return time.Duration(second) * time.Second, nil
	}
